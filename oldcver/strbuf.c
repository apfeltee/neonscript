...
void dyn_strutil_callboundscheckinsert(const StringBuffer* sbuf, size_t pos, const char* file, int line, const char* func)
{
    if(pos > sbuf->length)
    {
...
void dyn_strutil_callboundscheckreadrange(const StringBuffer* sbuf, size_t start, size_t len, const char* file, int line, const char* func)
{
...
StringBuffer* dyn_strbuf_makefromptr(StringBuffer* sbuf, size_t len)
{
    ...
    sbuf->data = (char*)malloc(sbuf->capacity);
    if(!sbuf->data)
    {
        return NULL;
    }
...
bool dyn_strbuf_fullreplace(StringBuffer* sb, const char* findstr, size_t findlen, const char* substr, size_t sublen)
{
    ...
    nbuf = dyn_strbuf_makeempty(needed);
    dyn_strbuf_appendstrn(nbuf, sb->data, sb->length);
