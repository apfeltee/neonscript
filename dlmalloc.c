...
#define DLMALLOC_USEDLMALLOC 0
//#define MSPACES 8
//#define DEBUG 1

#if defined(DLMALLOC_USEDLMALLOC) && (DLMALLOC_USEDLMALLOC == 1)
#ifndef WIN32
    #ifdef _WIN32
        #define WIN32 1
    #endif /* _WIN32 */
...
extern "C"
{
    ...
    */
    ...
    /*
      ...
      the given space.
    */
    static struct mallinfo mspace_mallinfo(mspace msp);
    #endif /* NO_MALLINFO */
    ...
    #define set_segment_flags(S, v)                                                                                                                                 \
        (((v) != IS_MMAPPED_BIT) ? (ABORT, (v)) :                                                                                                                   \
                                   (((S)->exec_offset = mmap_exec_offset((S)->base, (S)->size)),                                                                    \
                                    (mmap_exec_offset((S)->base + (S)->exec_offset, (S)->size) != (S)->exec_offset) ? (ABORT, (v)) :                                \
    ...
    if(tbase != CMFAIL)
    {
        ...
        else
        {
            ...
            while(sp != 0 && tbase != sp->base + sp->size)
                sp = sp->next;
            if(sp != 0 && !is_extern_segment(sp) && check_segment_merge(sp, tbase, tsize) && (get_segment_flags(sp) & IS_MMAPPED_BIT) == mmap_flag && segment_holds(sp, m->top))
            { /* append */
                sp->size += tsize;
            ...
            else
            {
                ...
                while(sp != 0 && sp->base != tbase + tsize)
                    sp = sp->next;
                if(sp != 0 && !is_extern_segment(sp) && check_segment_merge(sp, tbase, tsize) && (get_segment_flags(sp) & IS_MMAPPED_BIT) == mmap_flag)
                {
