...
{
    NeonHashTable* mthtabnumber;
    NeonHashTable* mthtabarray;
    NeonHashTable* mthtabstring;
    NeonHashTable* mthtabobject;
};
...
#if 0
    neon_writer_writeformat(state->stderrwriter, "markobject: (graycap=%d graycount=%d) at %p: ", state->gcstate.graycap, state->gcstate.graycount, (void*)object);
...
void neon_object_release(NeonState* state, NeonObject* object)
{
    ...
    switch(object->type)
    {
        ...
        case NEON_OBJ_INSTANCE:
        {
            NeonObjInstance* instance;
            instance = (NeonObjInstance*)object;
            neon_hashtable_release(instance->fields);
...
void neon_writer_printfunction(NeonWriter* wr, NeonObjScriptFunction* ofn)
{
    ...
    {
        neon_writer_writeformat(wr, "<function '%s'>", ofn->name->sbuf->data);
    }
}
...
void neon_writer_printarray(NeonWriter* wr, NeonObjArray* arr)
{
    ...
    for(i=0; i<asz; i++)
    {
        //neon_writer_writeformat(wr, "%ld: <(%d) %s>", i, arr->vala->values[i].type, neon_value_typename(arr->vala->values[i]));
...
void neon_writer_printobject(NeonWriter* wr, NeonValue value, bool fixstring)
{
    switch(neon_value_objtype(value))
    {
        case NEON_OBJ_USERDATA:
            {
                neon_writer_writeformat(wr, "<userdata %p>", neon_value_asuserdata(value));
            }
            break;
        case NEON_OBJ_BOUNDMETHOD:
            {
...
void neon_userdata_release(NeonObjUserdata* ud)
{
    ...
    neon_hashtable_release(ud->methods);
...
void neon_class_release(NeonObjClass* klass)
{
    NeonState* state;
    state = klass->pvm;
    neon_hashtable_release(klass->methods);
...
int neon_string_find(NeonStringBuffer* str, NeonStringBuffer* substr, size_t startidx, bool icase)
{
    bool found;
    bool matching;
    int index;
...
bool neon_string_split(NeonStringBuffer* str, NeonStringBuffer* sep, NeonValArray* dest)
{
    int matchidx;
    size_t sublen;
    size_t lastidx;
...
{
    return neon_map_setstr(map, name, neon_value_fromobject(neon_object_makenative(map->pvm, nat)));
}
...
void neon_map_release(NeonObjMap* map)
{
    ...
    if(map->shouldmark)
    {
        neon_hashtable_release(map->mapping);
    }
...
void neon_hashtable_release(NeonHashTable* table)
{
    NeonState* state;
    state = table->pvm;
...
{
    return neon_hashtable_setstr(tab, name, neon_value_fromobject(neon_object_makenative(tab->pvm, nat)));
}
...
int neon_dbg_dumpdisasm(NeonState* state, NeonWriter* wr, NeonChunk* chunk, int offset)
{
    ...
    {
        neon_writer_writeformat(wr, "%4d ", chunk->srclinenos[offset]);
    }
    instruction = chunk->bincode[offset];
    instrname = neon_prs_op2str(instruction);
    switch(instruction)
    {
...
NeonAstScanner* neon_lex_init(NeonState* state, const char* source)
{
    NeonAstScanner* scn;
...
NeonAstParser* neon_prs_init(NeonState* state)
{
    NeonAstParser* prs;
...
NeonObjScriptFunction* neon_prs_compilesource(NeonState* state, const char* source)
{
    NeonAstCompiler compiler;
    NeonObjScriptFunction* fn;
    state->parser = neon_prs_init(state);
    state->parser->pscn = neon_lex_init(state, source);
...
void neon_state_vraiseerror(NeonState* state, const char* format, va_list va)
{
    ...
    fputs("\n", stderr);
    count = 0;
    frmcnt = state->vmvars.framecount;
    for(itd = frmcnt - 1; itd >= 0; itd--)
    {
        frame = &state->vmvars.framevalues[itd];
...
void neon_state_defvalue(NeonState* state, const char* name, NeonValue val)
{
    ...
    neon_vm_stackpush(state, neon_value_fromobject(os));
    {
        ...
        {
            neon_hashtable_set(state->globals, neon_value_asstring(state->vmvars.stackvalues[0]), state->vmvars.stackvalues[1]);
        }
        neon_vm_stackpop(state);
    }
    neon_vm_stackpop(state);
...
void neon_vm_stackinit(NeonState* state)
{
    NeonVMStateVars* vm;
    vm = &state->vmvars;
    //vm->framevalues = (NeonCallFrame*)neon_mem_allocate(state, sizeof(NeonCallFrame), NEON_MAX_VMFRAMES, false);
    vm->framevalues = (NeonCallFrame*)malloc((NEON_MAX_VMFRAMES + 1) * sizeof(NeonCallFrame));

    //vm->stackvalues = (NeonValue*)neon_mem_allocate(state, sizeof(NeonValue), NEON_MAX_VMSTACK, false);
...
*/
/*
static void copyupval(NeonState* state, NeonObjUpvalue* to, NeonObjUpvalue* from, NeonValue* pslot, int32_t upidx)
{
    to->pvm = from->pvm;
    to->location = pslot;
...
*/
/*
static void copyscriptfunc(NeonObjScriptFunction* to, NeonObjScriptFunction* from)
{
    size_t i;
    to->arity = from->arity;
    to->upvaluecount = from->upvaluecount;
    to->chunk.count = from->chunk.count;
    for(i=0; i<from->chunk.count; i++)
    {
        to->chunk.bincode[i] = from->chunk.bincode[i];
    }
}
*/
/*
...
*/
/*
static void copyclosure(NeonState* state, NeonObjClosure* to, NeonObjClosure* from)
{
    size_t i;
    NeonValue* pslot;
    NeonObjUpvalue* upto;
    NeonObjUpvalue* upfrom;
    for(i=0; i<from->upvaluecount; i++)
    {
        if(from->upvalues[i] == NULL)
        {
            to->upvalues[i] = NULL;
        }
        else
        {
            upfrom = from->upvalues[i];
            pslot = &state->vmvars.stackvalues[upfrom->upindex];
            to->upvalues[i] = malloc(sizeof(NeonObjUpvalue));
            copyupval(state, to->upvalues[i], from->upvalues[i], pslot, upfrom->upindex);
        }
    }
    to->upvaluecount = from->upvaluecount;
    copyscriptfunc(to->innerfn, from->innerfn);
}
*/
/*
* grows vmvars.(stack|frame)values, respectively.
...
void neon_vm_stackresize(NeonState* state, size_t needed)
{
    ...
    */
    {
        oldsz = state->vmvars.stackcapacity;
        newsz = oldsz + nforvals;
        allocsz = ((newsz + 1) * sizeof(NeonValue));
        #if 1
            oldbuf = state->vmvars.stackvalues;
        ...
        #endif
        state->vmvars.stackvalues = (NeonValue*)newbuf;
        state->vmvars.stackcapacity = newsz;
    }
    {
        oldsz = state->vmvars.framecapacity;
        newsz = oldsz + needed;
        allocsz = ((newsz + 1) * sizeof(NeonCallFrame));
        #if 1
            oldbuf = state->vmvars.framevalues;
            newbuf = realloc(oldbuf, allocsz);
            if(newbuf == NULL)
            {
        ...
        #endif
        state->vmvars.framevalues = (NeonCallFrame*)newbuf;
        state->vmvars.framecapacity = newsz;
    }
    /*
...
{
    if(state->vmvars.stackcapacity < (pos + 1))
    {
...
{
    if(state->vmvars.framecapacity < (pos + 1))
    {
        neon_vm_stackgrow(state);
    }
}
...
{
    neon_vm_stackmaybegrow(state, state->vmvars.stacktop + 0);
...
{
    state->objvars.mthtabnumber = neon_hashtable_make(state);
    state->objvars.mthtabarray = neon_hashtable_make(state);
    state->objvars.mthtabstring = neon_hashtable_make(state);
    state->objvars.mthtabobject = neon_hashtable_make(state);
}

void neon_state_releaseobjvars(NeonState* state)
{
    neon_hashtable_release(state->objvars.mthtabnumber);
    neon_hashtable_release(state->objvars.mthtabarray);
    neon_hashtable_release(state->objvars.mthtabstring);
    neon_hashtable_release(state->objvars.mthtabobject);
}
...
void neon_state_free(NeonState* state)
{
    ...
    neon_hashtable_release(state->globals);
    neon_hashtable_release(state->strings);
    state->initstring = NULL;
    neon_vm_gcfreelinkedobjects(state);
    if(state->parser != NULL)
    {
        neon_prs_release(state->parser);
    }
    neon_state_releaseobjvars(state);
    free(state->vmvars.framevalues);
    free(state->vmvars.stackvalues);
    free(state);
}
...
static inline int32_t neon_vmbits_readbyte(NeonState* state)
{
    int32_t r;
    int32_t* vp;
    vp = state->vmvars.currframe->instrucptr;
    r = *vp;
    state->vmvars.currframe->instrucptr++;
    return r;
}
...
static inline uint16_t neon_vmbits_readshort(NeonState* state)
{
    int32_t a;
    int32_t b;
    a = state->vmvars.currframe->instrucptr[0];
    b = state->vmvars.currframe->instrucptr[1];
    state->vmvars.currframe->instrucptr += 2;
...
bool neon_vmbits_callclosure(NeonState* state, NeonValue receiver, NeonObjClosure* closure, int argc)
{
    ...
    frame->instrucptr = closure->innerfn->chunk.bincode;
...
NeonHashTable* neon_value_getmethodtable(NeonState* state, NeonValue val)
{
    ...
    {
        return state->objvars.mthtabnumber;
    }
    else if(neon_value_isarray(val))
    {
        return state->objvars.mthtabarray;
    }
    else if(neon_value_isstring(val))
    {
        return state->objvars.mthtabstring;
    }
    return NULL;
}
...
bool neon_value_getcallable(NeonState* state, NeonValue receiver, NeonObjString* name, NeonValue* dest)
{
    ...
    else
    {
        ...
        /* if that is also not the case, see if $mthtabobject has a matching method */
        if(neon_hashtable_get(state->objvars.mthtabobject, name, dest))
        {
            return true;
        }
...
static inline void neon_vm_debugprintinstruction(NeonState* state, NeonWriter* owr, NeonCallFrame* frame)
{
    ...
    ofs = (int)(frame->instrucptr - frame->closure->innerfn->chunk.bincode) - 1;
            ...
            op_case(NEON_OP_PRIMADD)
                {
                    ...
                    {
                        neon_state_raiseerror(state, "expected <number>|<number> or <string>|<string>, but got (#0|#1) <%s>|<%s>", neon_value_typename(peek1), neon_value_typename(peek2));
                        return NEON_STATUS_RUNTIMEERROR;
                    }
                }
                vm_breakouter();
            ...
            op_case(NEON_OP_JUMPNOW)
                {
                    ...
                    state->vmvars.currframe->instrucptr += offset;
                }
                vm_breakouter();
...
NeonStatusCode neon_state_runsource(NeonState* state, const char* source)
{
    ...
    neon_vmbits_callclosure(state, neon_value_makenil(), closure, 0);
...
static NeonValue objfn_number_chr(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    ...
    (void)argc;
    (void)argv;
    dw = selfval.as.number;
    ch = (char)dw;
    os = neon_string_copy(state, &ch, 1);
    return neon_value_fromobject(os);
}

static NeonValue objfn_string_length(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    NeonObjString* os;
    (void)state;
    (void)argc;
...
static NeonValue objfn_string_chars(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    size_t i;
    NeonObjArray* oa;
    NeonObjString* os;
...
static NeonValue objfn_string_split(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    NeonValue vsep;
    NeonObjArray* oa;
    NeonObjString* selfstr;
    ...
    if(sep == NULL)
    {
        return objfn_string_chars(state, selfval, argc, argv);
    }
    selfstr = neon_value_asstring(selfval);
    oa = neon_array_make(state);
    neon_string_split(selfstr->sbuf, sep->sbuf, oa->vala);
    return neon_value_fromobject(oa);
}

static NeonValue objfn_object_dump(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    NeonObjString* res;
    NeonWriter* wr;
    (void)argc;
...
static NeonValue objfn_array_push(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    int i;
    (void)state;
    NeonObjArray* oa;
...
static NeonValue objfn_array_count(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    (void)state;
    NeonObjArray* oa;
    (void)argc;
...
static NeonValue objfn_array_pop(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    (void)state;
    NeonValue val;
    NeonObjArray* oa;
...
static NeonValue objfn_array_set(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    long idx;
    NeonValue putval;
    NeonValue idxval;
...
static NeonValue objfn_array_erase(NeonState* state, NeonValue selfval, int argc, NeonValue* argv)
{
    long idx;
    NeonValue idxval;
    NeonObjArray* oa;
...
static NeonValue objfn_file_readfile(NeonState* state, NeonValue rec, int argc, NeonValue* argv)
{
    bool withmin;
    size_t flen;
    size_t minsz;
...
static NeonValue objfn_file_open(NeonState* state, NeonValue rec, int argc, NeonValue* argv)
{
    FILE* fh;
    NeonValue va;
    NeonObjUserdata* ud;
...
int main(int argc, char** argv)
{
    ...
    {
        tab = state->objvars.mthtabobject;
        map = neon_object_makemapfromtable(state, tab, false);
        neon_hashtable_setstrfunction(tab, "dump", objfn_object_dump);
        neon_state_defvalue(state, "Object", neon_value_fromobject(map));        
    }
    {
        tab = state->objvars.mthtabnumber;
        neon_hashtable_setstrfunction(tab, "chr", objfn_number_chr);
    }
    {
        tab = state->objvars.mthtabstring;
        neon_hashtable_setstrfunction(tab, "length", objfn_string_length);
        neon_hashtable_setstrfunction(tab, "split", objfn_string_split);
        neon_hashtable_setstrfunction(tab, "chars", objfn_string_chars);
        #if 0
        neon_hashtable_setstrfunction(tab, "lstrip", objfn_string_leftstrip);
        neon_hashtable_setstrfunction(tab, "rstrip", objfn_string_rightstrip);
        neon_hashtable_setstrfunction(tab, "strip", objfn_string_strip);
        neon_hashtable_setstrfunction(tab, "toUpper", objfn_string_toupper);
        neon_hashtable_setstrfunction(tab, "tolower", objfn_string_tolower);
        

        #endif
    }
    {
        tab = state->objvars.mthtabarray;
        neon_hashtable_setstrfunction(tab, "count", objfn_array_count);
        neon_hashtable_setstrfunction(tab, "length", objfn_array_count);
        neon_hashtable_setstrfunction(tab, "push", objfn_array_push);
        neon_hashtable_setstrfunction(tab, "append", objfn_array_push);
        neon_hashtable_setstrfunction(tab, "erase", objfn_array_erase);
        neon_hashtable_setstrfunction(tab, "set", objfn_array_set);
        neon_hashtable_setstrfunction(tab, "pop", objfn_array_pop);
    }
    {
        map = neon_object_makemap(state);
    ...
    {
        map = neon_object_makemap(state);
        neon_map_setstrfunction(map, "readfile", objfn_file_readfile);
        neon_map_setstrfunction(map, "open", objfn_file_open);
